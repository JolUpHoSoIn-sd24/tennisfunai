...
class BallDetector:
    ...
    def infer_model_for_single_frame(self, frame, frame_prev, frame_preprev, ball_track):
        prev_pred = ball_track[-1]
        
        img = cv2.resize(frame, (self.width, self.height))
        img_prev = cv2.resize(frame_prev, (self.width, self.height))
        img_preprev = cv2.resize(frame_preprev, (self.width, self.height))
        imgs = np.concatenate((img, img_prev, img_preprev), axis=2)
        imgs = imgs.astype(np.float32)/255.0
        imgs = np.rollaxis(imgs, 2, 0)
        inp = np.expand_dims(imgs, axis=0)
